(() => {
  ...
  window.DAYBREAK_RENDER_ISSUES = body => {
    ...
    summary.append(make('span', '', 'Open issues you created'), make('span', '', saved ? `${data.total ?? issues.length} open` : ''));
    body.append(summary);
    const list = make('div', 'github-list');
    list.setAttribute('aria-label', 'Your open GitHub issues');
    issues.forEach(issue => {
      let destination;
      try { destination = new URL(issue.url); } catch { return; }
      if (destination.origin !== 'https://github.com' || !/^\/[^/]+\/[^/]+\/issues\/\d+$/.test(destination.pathname)) return;
      const row = make('a', 'github-issue');
      row.href = destination.href;
      row.append(make('span', 'github-dot', '◉'));
      const copy = make('span', 'github-copy');
      copy.append(make('span', 'event-title', issue.title), make('span', 'github-repo', `${issue.repository} · #${issue.number}`));
      const labels = make('span', 'github-labels');
      (Array.isArray(issue.labels) ? issue.labels : []).slice(0, 2).forEach(label => labels.append(make('span', 'github-label', label)));
      copy.append(labels); row.append(copy); list.append(row);
    });
    if (!issues.length) list.append(make('p', 'agenda-empty', saved ? 'A clear list. You have no open issues.' : 'Issues are unavailable right now. The local helper will try again.'));
    body.append(list);
    const footer = make('div', 'agenda-foot');
    let status = !saved ? 'Waiting for GitHub' : data.status !== 'connected' ? 'Sync unavailable · saved issues' : !fresh ? 'Sync paused · saved issues' : 'Live · updated ' + new Date(date).toLocaleTimeString('en-GB', {hour:'2-digit',minute:'2-digit'});
    ...
    open.href = 'https://github.com/issues?q=' + encodeURIComponent('is:issue is:open author:' + author);
    footer.append(open); body.append(footer);
  };
  let loading = false;
  function refresh() {
    ...
    script.onload = script.onerror = () => {
      ...
      body.replaceChildren(); window.DAYBREAK_RENDER_ISSUES(body);
