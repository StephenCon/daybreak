...
const context={window:{matchMedia:()=>({matches:false})},document:{createElement:()=>({}),head:{append(){}}},crypto,URL};
...
const legacy=context.window.test.defaults();delete legacy.palette;assert.equal(context.window.test.validate(legacy).palette,'daybreak');
for(const palette of Object.keys(context.window.DAYBREAK_THEMES.themes)){const saved=context.window.test.defaults();saved.palette=palette;assert.equal(context.window.test.validate(JSON.parse(JSON.stringify(saved))).palette,palette);assert.equal(saved.theme,'system')}
assert.throws(()=>context.window.test.validate({...context.window.test.defaults(),palette:'invalid'}));
class Element{constructor(tag){this.tag=tag;this.children=[]}append(...nodes){this.children.push(...nodes)}setAttribute(){}get text(){return [this.textContent||'',...this.children.map(c=>c.text)].join(' ')}}
const ui={window:{},document:{createElement:tag=>new Element(tag),addEventListener(){}},setInterval(){},URL,Date};vm.createContext(ui);
vm.runInContext(fs.readFileSync('github-widget.js','utf8'),ui);
ui.window.DAYBREAK_GITHUB={status:'connected',updatedAt:new Date().toISOString(),total:40,issues:[{url:'https://github.com/example/demo/issues/1',title:'<script>not executable</script>',repository:'example/demo',number:1,labels:['bug']},{url:'javascript:alert(1)',title:'bad'}]};
let body=new Element('div');ui.window.DAYBREAK_RENDER_ISSUES(body);
